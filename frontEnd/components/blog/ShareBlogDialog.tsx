...
} from "@/app/lib/blog-share";

interface ShareBlogDialogProps {
  isOpen: boolean;
  onClose: () => void;
  payload: BlogSharePayload | null;
}

export default function ShareBlogDialog({
  isOpen,
  onClose,
  payload,
}: ShareBlogDialogProps) {
  const [copiedLink, setCopiedLink] = useState(false);
  const [copiedMessage, setCopiedMessage] = useState(false);

  const shareText = useMemo(
    () => (payload ? buildBlogShareMessage(payload) : ""),
    [payload]
  );
  const shareUrl = useMemo(
    () => (payload ? getBlogShareUrl(payload.slug) : ""),
    [payload]
  );
  ...
  const channels = [
    ...
    },
    {
      ...
      icon: Send,
      className:
        "bg-sky-50 dark:bg-sky-900/30 text-sky-700 dark:text-sky-300 hover:bg-sky-100",
    ...
    },
    {
      ...
      icon: Share2,
      className:
        "bg-gray-50 dark:bg-gray-700 text-gray-800 dark:text-gray-200 hover:bg-gray-100",
    ...
    },
    {
      ...
      onClick: () => openWindow(facebookShareUrl(shareUrl)),
    },
    {
      id: "email",
      label: "Email",
      ...
      onClick: () =>
        openWindow(emailShareUrl(`Blog: ${payload.title}`, shareText)),
    },
  ];
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
      <div
        ...
        role="dialog"
        aria-labelledby="share-blog-title"
      >
        ...
        <div className="p-4 space-y-4 overflow-y-auto">
          ...
          {canUseNativeShare() && (
            <button
              type="button"
              onClick={() => void nativeShareBlog(payload)}
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">Or share on:</p>
          <div className="grid grid-cols-2 gap-2">
            {channels.map((ch) => (
              <button
                key={ch.id}
